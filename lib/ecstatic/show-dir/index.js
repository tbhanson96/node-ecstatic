    ...
    fs.stat(dir, (statErr, stat) => {
      ...
      fs.readdir(dir, (readErr, _files) => {
        ...
        function render(dirs, renderFiles, lolwuts) {
          ...
          html += `${[`<form action="${he.encode(pathname)}" method="post" enctype="multipart/form-data">`,
                      `<input type="file" name="${he.encode(pathname)}">`,
          ...
          html += `<br><address>Node.js ${
            process.version
        }/ <a href="https://github.com/tbhanson96/node-ecstatic-upload">ecstatic-upload</a> ` +
            `server running @ ${
